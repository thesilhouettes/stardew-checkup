...
window.onload = function () {
  ...
  function getSectionHeader(
    ...
    var output =
      '<div class="collapsible" id="wrap_' +
      anchor +
      '"><h3>' +
      title +
      "</h3>";
  ...
  function collapsibleWrap(saveInfo, title, output, version) {
    return "<h4>PLACEHOLDER</h4>" + output;
  }
  ...
  function parsePlayerMoney(player, saveInfo, meta) {
    ...
    output +=
      '<div class="' + meta.anchor + "_summary " + meta.sum_class + '">';
    output +=
      '<span class="result">' +
  ...
  function parseMuseum(xmlDoc, saveInfo) {
    ...
    output +=
      donated_count >= 60
        ...
        : getMilestoneString(
            "Donate enough items (60) to get the Rusty Key",
            0
          ) +
          (60 - donated_count) +
          " more";
  ...
  function parsePlayerMuseum(player, saveInfo, meta) {
    ...
    if (donated_count < museum_count || found_art + found_min < museum_count) {
      ...
      if (need_art.length > 0) {
        output +=
          "<li>Artifacts<ol>" + need_art.sort().join("") + "</ol></li>\n";
      }
      if (need_min.length > 0) {
        output +=
          "<li>Minerals<ol>" + need_min.sort().join("") + "</ol></li>\n";
      }
      output += "</ul></span></div>";
    }
